...
const uploadInlineImages = async (
  ...
  for await (const child of parsedContent.childNodes) {
    if (child.nodeName.toLowerCase() === 'img') {
      ...
      // const blobUrl = await fetchImageSource(
      //   image,
      ...
      console.log('3. Leah: ::: single new imageNode', imageResult);
      console.log('3.1 Leah: ::: single child', child);
...
const uploadInlineImage = async (
  ...
  blob: Blob,
  fileName: string
): Promise<UploadChatImageResult | undefined> => {
  console.log(client, blob, fileName);
