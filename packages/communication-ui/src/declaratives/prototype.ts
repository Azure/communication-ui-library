...
export interface WithState {
  ...
  onStateChange(handler: (state: ChatThreadState) => void): void;
}
 ...
 */
export interface ChatThreadClientWithState extends ChatThreadClient, WithState {}
 ...
 * @param setState
 */
const proxyListMessagesWithState = (
 ...
 */
class ProxyChatThreadClientWithState implements ProxyHandler<ChatThreadClient>, WithState {
  ...
  public get<P extends keyof ChatThreadClient>(target: ChatThreadClient, prop: P): any {
    switch (prop) {
      case 'listMessages': {
        return proxyListMessagesWithState(target, this.getState, this.setState);
      }
      default:
...
/**
 * Helper function that creates a ChatThreadClientWithState that proxies the given ChatThreadClient.
 *
 * @param chatThreadClient
 */
export const proxyChatThreadClientWithState = (chatThreadClient: ChatThreadClient): ChatThreadClientWithState => {
  return new Proxy(chatThreadClient, new ProxyChatThreadClientWithState()) as ChatThreadClientWithState;
};

/**
- Usage:

const chatThreadClientWithState = proxyChatThreadClientWithState(chatClient.createChatThread());
chatThreadClientWithState.onStateChange((state: ChatThreadState) => console.log('onStateChange', state));
console.log('getState', chatThreadClientWithState.getState());
*/
