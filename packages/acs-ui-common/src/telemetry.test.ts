...
test('application ID helper works for CallWithChatComposite for all versions in use', () => {
  expect(_getAppliationIdImpl('1.0.0', 'callWithChat')).toEqual('acr13/1.0.0');
  expect(_getAppliationIdImpl('1.0.1', 'callWithChat')).toEqual('acr13/1.0.1');
  expect(_getAppliationIdImpl('99.99.99', 'callWithChat')).toEqual('acr13/99.99.99');
  expect(_getAppliationIdImpl('99.99.99-beta.99', 'callWithChat')).toEqual('acr13/99.99.99-beta.99');
  expect(_getAppliationIdImpl('1.0.0-alpha-202108050010.0', 'callWithChat')).toEqual('acr13/1.0.0-alpha');
  expect(applicationIdFormat.test(_getAppliationIdImpl('1.0.0-alpha-202108050010.0'))).toBe(true);
});

test('application ID helper works for all composites with first stable version', () => {
  expect(_getAppliationIdImpl('1.0.0', 'call')).toEqual('acr11/1.0.0');
  expect(_getAppliationIdImpl('1.0.0', 'chat')).toEqual('acr12/1.0.0');
  expect(_getAppliationIdImpl('1.0.0', 'callWithChat')).toEqual('acr13/1.0.0');
});
