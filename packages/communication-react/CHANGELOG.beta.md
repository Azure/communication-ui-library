...
### Features
- Change notifications component name and revert changes in video effect panel ([commit](https://github.com/azure/communication-ui-library/commit/not available) by 96077406+carocao-msft@users.noreply.github.com)
- Transition to breakout room when it is open and return to main meeting when breakout room is closed ([commit](https://github.com/azure/communication-ui-library/commit/not available) by 79475487+mgamis-msft@users.noreply.github.com)
- This release brings major enhancements to the rich text editor feature.  -- We now support enabling rich text editor when using either the Chat Composite or the Call with Chat Composite.-- We have also added support for sending/editing messages that include images when in a 'Teams Interop' meeting using the Call with Chat Composite. -- We now support setting a Text Only MessagingPolicy on the ChatComposite. This prevents attachments and inline images to be added as part of a message in Send Box and Message Edit Box.-- There is also an additional onPaste Callback that can be used to customize content pasted to rich text editor. ([commit](https://github.com/azure/communication-ui-library/commit/not available) by 73612854+palatter@users.noreply.github.com)
- Add new component for incoming call notifications ([commit](https://github.com/azure/communication-ui-library/commit/not available) by dmceachern@microsoft.com)
- Add package that includes new Javascript loaders to allow for Cross platform usage of the UI composites ([commit](https://github.com/azure/communication-ui-library/commit/not available) by 94866715+dmceachernmsft@users.noreply.github.com)
- add other composites to loaders ([commit](https://github.com/azure/communication-ui-library/commit/not available) by dmceachern@microsoft.com)
- Update Declarative teams callAgent to use its own typing ([commit](https://github.com/azure/communication-ui-library/commit/not available) by dmceachern@microsoft.com)
...
- Error notifications in composite ([commit](https://github.com/azure/communication-ui-library/commit/not available) by 96077406+carocao-msft@users.noreply.github.com)
- Change Close to Dismiss for button aria label ([commit](https://github.com/azure/communication-ui-library/commit/not available) by 96077406+carocao-msft@users.noreply.github.com)
- Enable local screen share stream ([commit](https://github.com/azure/communication-ui-library/commit/not available) by 96077406+carocao-msft@users.noreply.github.com)
- Update CTE typing to reflect TeamsIncomingCalls ([commit](https://github.com/azure/communication-ui-library/commit/not available) by dmceachern@microsoft.com)
- Migrate Compliance banner to new notification system ([commit](https://github.com/azure/communication-ui-library/commit/not available) by 96077406+carocao-msft@users.noreply.github.com)
- We are excited to announce that the Azure Communication Services Web UI Library now supports the Soft Mute feature. This feature enables users to mute other users in a call. If a user has been soft muted, they retain the ability to unmute themselves. Developers can use this functionality today through our composites (e.g CallComposite, CallWithChatComposite) as well as through components (e.g VideoGallery). ([commit](https://github.com/azure/communication-ui-library/commit/not available) by edwardlee@microsoft.com)
- Add IncomingCallStack component to manage incoming calls and add a component binding option for this component ([commit](https://github.com/azure/communication-ui-library/commit/not available) by dmceachern@microsoft.com)
- remove icons from stable for inbound ([commit](https://github.com/azure/communication-ui-library/commit/not available) by dmceachern@microsoft.com)
- add customization options to stack component ([commit](https://github.com/azure/communication-ui-library/commit/not available) by dmceachern@microsoft.com)
- Add customization API's to the incoming call toast notification ([commit](https://github.com/azure/communication-ui-library/commit/not available) by dmceachern@microsoft.com)
- Fix compliance banner showing recording stopped when starting recording ([commit](https://github.com/azure/communication-ui-library/commit/not available) by 96077406+carocao-msft@users.noreply.github.com)
- Update notifications storybook doc to include dismissed notifications handling ([commit](https://github.com/azure/communication-ui-library/commit/not available) by 96077406+carocao-msft@users.noreply.github.com)
- Azure Communication Services UI Library is adding support for improved in-call notifications. In call notifications are essential for providing users with timely and relevant information about their calling experience. Whether it is an error message, a mute status, or a network quality indicator, notifications can help users troubleshoot issues and improve their communication. The new feature of ACS UI Library simplifies the display and management of multiple notifications in a consistent and user-friendly way. The in-call notification feature introduces a streamlined UI experience for displaying errors and notifications in the calling environment. ([commit](https://github.com/azure/communication-ui-library/commit/not available) by 96077406+carocao-msft@users.noreply.github.com)
- Fix bug where isMuted banner not showing up when compliance banners are up, also make sure notifications are ordered from earliest to latest ([commit](https://github.com/azure/communication-ui-library/commit/not available) by 96077406+carocao-msft@users.noreply.github.com)
### Improvements
...
- Update @azure/communication-calling to 1.27.1-beta.1 and 1.26.2 ([commit](https://github.com/azure/communication-ui-library/commit/not available) by 2684369+JamesBurnside@users.noreply.github.com)
### Bug Fixes
