...
describe('registerAttachmentUploads()', () => {
  test('should create file uploads in state', () => {
    ...
    adapter.registeractiveAttachmentUploads(generateFiles(1));
  ...
  test('should append file uploads to state without changing existing file uploads', () => {
    const chatContext = createChatContext();
    const adapter = new AzureCommunicationAttachmentUploadAdapter(chatContext);
    const AttachmentUploads_1 = adapter.registeractiveAttachmentUploads(generateFiles(1));
  ...
  test('should remove erroneous file uploads from state', () => {
    const chatContext = createChatContext();
    const adapter = new AzureCommunicationAttachmentUploadAdapter(chatContext);
    const AttachmentUploads = adapter.registeractiveAttachmentUploads(generateFiles(2));
    chatContext.setState(
      produce(chatContext.getState(), (draft: ChatAdapterState) => {
        if (draft.attachmentUploads?.[AttachmentUploads[0].id]) {
          // TODO: pass error message to send box
          // draft.attachmentUploads[AttachmentUploads[0].id].error = { message: 'Sample Error Message', timestamp: Date.now() };
        }
      })
    );
    adapter.registeractiveAttachmentUploads(generateFiles(2));
...
describe('clearAttachmentUploads()', () => {
  test('should remove all file uploads from state', () => {
    const chatContext = createChatContext();
    const adapter = new AzureCommunicationAttachmentUploadAdapter(chatContext);
    adapter.registeractiveAttachmentUploads(generateFiles(5));
...
describe('cancelAttachmentUpload()', () => {
  test('should remove file upload from state', () => {
    const chatContext = createChatContext();
    const adapter = new AzureCommunicationAttachmentUploadAdapter(chatContext);
    const attachmentUploads = adapter.registeractiveAttachmentUploads(generateFiles(5));
