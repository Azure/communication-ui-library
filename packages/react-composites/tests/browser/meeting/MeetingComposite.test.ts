...
test.describe('Meeting Composite Meeting Page Tests', () => {
  ...
  test('Chat messages are displayed correctly', async ({ pages }) => {
    ...
    await waitForMessageDelivered();
    await waitForSelector(pages[1], `[data-ui-status="delivered"]`);
    // Test page 0 has both sent message and received message
    await pages[0].bringToFront();
    await waitForSelector(pages[0], `[data-ui-status="seen"]`);
    await pages[1].bringToFront();
    await waitForSelector(pages[1], `[data-ui-status="seen"]`);
