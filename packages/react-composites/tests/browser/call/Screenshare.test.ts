...
test.describe('Screenshare tests', async () => {
  ...
  test(
    'Remote screen share stream should be displayed in grid area of VideoGallery. ' +
      'All remote participants should be HorizontalGallery spanning multiple pages. ' +
      'Navigation buttons should work.',
    async ({ pages, serverUrl }) => {
      const page = pages[0];
      const testRemoteParticipants = [
        {
          displayName: 'Pardeep Singh'
