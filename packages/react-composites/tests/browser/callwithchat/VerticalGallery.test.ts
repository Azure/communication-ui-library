...
test.describe('VerticalGallery tests', async () => {
  ...
  test('VerticalGallery should work in right-to-left', async ({ page, serverUrl }) => {
    ...
    await waitForSelector(page, dataUiId(IDS.videoGallery));
    expect(await stableScreenshot(page)).toMatchSnapshot('vertical-gallery-in-rtl-with-1-audio-participant.png');
  });

  test('VerticalGallery should have multiple audio participants spanning multiple pages. Navigation buttons should work.', async ({
    page,
    serverUrl
  }) => {
    const paul = defaultMockRemoteParticipant('Paul Bridges');
    ...
    expect(await stableScreenshot(page)).toMatchSnapshot('vertical-gallery-with-many-audio-participants-on-page-2.png');
  ...
  test('VerticalGallery should have 1 PSTN participant in the vertical gallery', async ({ page, serverUrl }) => {
    const paul = defaultMockRemoteParticipant('Paul Bridges');
    addVideoStream(paul, true);
    ...
    await waitForSelector(page, dataUiId(IDS.videoGallery));
    expect(await stableScreenshot(page)).toMatchSnapshot('vertical-gallery-with-joining-participant.png');
  });

  /* @conditional-compile-remove(PSTN-calls) */
  test('VerticalGallery should have multiple audio participants and 1 PSTN participant', async ({
    page,
    serverUrl
  }) => {
    const paul = defaultMockRemoteParticipant('Paul Bridges');
    ...
    expect(await stableScreenshot(page)).toMatchSnapshot(
      'vertical-gallery-with-joining-participant-with-audio-participants.png'
    );
  });

  /* @conditional-compile-remove(PSTN-calls) @conditional-compile-remove(pinned-participants) */
  test('VerticalGallery should have multiple audio participants and 1 PSTN participant on second page', async ({
    page,
    serverUrl
  }) => {
    const paul = defaultMockRemoteParticipant('Paul Bridges');
    ...
    await pageClick(page, dataUiId(IDS.overflowGalleryRightNavButton));
    expect(await stableScreenshot(page)).toMatchSnapshot(
      'vertical-gallery-with-joining-participant-with-multi-page.png'
    );
  });

  /* @conditional-compile-remove(PSTN-calls) */
  test('VerticalGallery should have 2 video participants during screenshare and 1 PSTN participant', async ({
    page,
    serverUrl
  }) => {
    const paul = defaultMockRemoteParticipant('Paul Bridges');
    ...
    await waitForSelector(page, dataUiId(IDS.videoGallery));
    expect(await stableScreenshot(page)).toMatchSnapshot(
      'vertical-gallery-with-joining-participant-with-screen-share-and-video.png'
    );
  });
  ...
  */
  test('Vertical gallery Should have 1 PSTN and 1 1-N participants', async ({ page, serverUrl }) => {
    const reina = defaultMockRemoteParticipant('Reina Takizawa');
    ...
    await waitForSelector(page, dataUiId(IDS.videoGallery));
    expect(await stableScreenshot(page)).toMatchSnapshot('vertical-gallery-with-2-joining-participants.png');
  });

  /* @conditional-compile-remove(PSTN-calls) */
  test('Vertical gallery Should have 1 PSTN and 1 On Hold participant', async ({ page, serverUrl }) => {
    const reina = defaultMockRemoteParticipant('Reina Takizawa');
    reina.state = 'Hold';
    const phoneUser = defaultMockRemotePSTNParticipant('+15555555555');
    phoneUser.state = 'Ringing';
    ...
    await waitForSelector(page, dataUiId(IDS.videoGallery));
    expect(await stableScreenshot(page)).toMatchSnapshot('vertical-gallery-with-1-joining-1-hold-participants.png');
  });
});
