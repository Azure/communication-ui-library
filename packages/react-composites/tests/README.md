...
### Stress testing via GitHub action

There is also a [GitHub action to help stress test tests](https://github.com/Azure/communication-ui-library/actions/workflows/stress-test-ui-tests.yml). When changing a test significantly, it is good practice to trigger a stress test workflow, and add a link to the running workflow to your PR description.

You can stress just the affected test by picking your branch to run tests from and providing an expression to constrain the test to run (similar to what you would use with `playwright -g`):

![Image showing how to stress test a single test on a PR](./static/stress-test-one-test.png)
