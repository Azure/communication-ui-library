...
<Meta title="Concepts/Ad-hoc calling" />
...
of the [CallAdapter](?path=/docs/composite-adapters--page#call-adapter) when creating a call. Now when the [CallComposite](?path=/docs/composites-call-basicexample--basic-example) starts up it will call the Teams voice application
...
This array should be given to the `targetCallees` property of the [CallAdapter](?path=/docs/composite-adapters--page#call-adapter) when creating a call. Like with the bot, the
