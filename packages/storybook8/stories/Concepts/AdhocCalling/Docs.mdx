...
of the [CallAdapter](?path=/docs/composites-adapters--docs#calladapter) when creating a call. Now when the [CallComposite](?path=/docs/composites-callcomposite--docs) starts up it will call the Teams voice application
...
This array should be given to the `targetCallees` property of the [CallAdapter](?path=/docs/composites-adapters--docs#calladapter) when creating a call. Like with the bot, the
