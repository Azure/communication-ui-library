...
<Canvas mdxSource={IncomingCallNotificationExampleDocsOnly} source={{ code: IncomingCallNotificationText }} />
## Styling Depending on your use of `IncomingCallNotification` and the `IncomingCallStack` you might want to customize
your notification's appearence to match your needs. Both components includes API's to adjust the CSS on many different
elements of the notification. Below is an example of how you can customize `IncomingCallNotification`.

<Canvas
  mdxSource={IncomingCallNotificationStylingExampleDocsOnly}
