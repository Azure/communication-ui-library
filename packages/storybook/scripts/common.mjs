...
import { exec_output } from '../../../common/scripts/lib/exec.mjs';
...
export async function getExtraEnv() {
  const nodeVersion = await exec_output(quote(['node', '-v']));
  const nodeMajorVersion = parseInt(nodeVersion.split('.')[0].slice(1));
