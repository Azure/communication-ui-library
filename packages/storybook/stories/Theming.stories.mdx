...
import ThemedCompositesText from '!!raw-loader!./snippets/ThemedComposites.snippet.tsx';
...
The imports `lightTheme` and `darkTheme` are [Theme](https://aka.ms/fluentui#/controls/web/references/theme) objects and are
available from package `@azure/communication-react`.
...
ACS UI Library composites are also themed with a light theme by default and are themed through the `fluentTheme` property.
Example code is shown below.

<Source code={ThemedCompositesText} />
...
to create your theme. Please ensure that the Accessibility checker on the same page passes when creating your own theme.
...
The ACS UI Library and Fluent UI components are themed with a palette values from the
...
But if you wish to override the mapping, we advise that you use the `useTheme` hook from `@azure/communication-react` and
...
Should you override your own mappings, please note that ACS UI Library components abide by the general Fluent UI
