...
import FluentThemeProviderSnippetText from '!!raw-loader!./snippets/FluentThemeProviderSnippet.tsx';
import { FluentThemeProviderSnippet } from './snippets/FluentThemeProviderSnippet';
import DarkControlBarText from '!!raw-loader!./snippets/DarkControlBar.tsx';
import { DarkControlBar } from './snippets/DarkControlBar';
import ThemedButtonText from '!!raw-loader!./snippets/ThemedButton.tsx';
import { ThemedButton } from './snippets/ThemedButton';
...
ACS UI SDK use components and icons from both [Fluent UI](https://developer.microsoft.com/en-us/fluentui#/controls/web)
...
To use the FluentThemeProvider, simply import it from `@azure/communication-ui` and wrap it around
...
1. `FluentThemeProvider` loads the icons defined by the props such as `videoButtonProps`
2. `FluentThemeProvider` applies theme (light theme by default) to the `ControlBar`

To illustrate how another theme can be applied, import `darkTheme` from `@azure/communication-ui`
...
If you wish to override the mapping, we advise that you use the `useTheme` hook provided by Fluent UI.
