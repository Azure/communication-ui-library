...
2. `FluentThemeProvider` applies theme (light theme by default) to the `ControlBarComponent`
...
```typescript
import { darkTheme } from '@azure/communication-ui';
...
The dark theme is applied to the `ControlBarComponent`. The icon colors in `DefaultButton` are themed
...
![Accessibility checker pass](/stories/images/accessibility_checker_pass.png)
...
const MyButton = () => JSX.Element {
    ...
    return (
        <DefaultButton
            text="exampleButton"
            styles={{root: {background: theme.palette.neutralLight}}}
        />
    );
}
