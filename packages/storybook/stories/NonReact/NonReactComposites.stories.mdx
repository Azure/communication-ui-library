...
```tsx
  const chatAdapter =   The for composite which allows you control composite externally,
    await chatComposite.loadChatComposite( // This function will be exposed to window.chatComposite once you import composite bundle
      ...
      props // Object for all props(except adapter) of composite, optional, check composite page for more details
    )
```

```tsx
  const callAdapter =   The for composite which allows you control composite externally,
