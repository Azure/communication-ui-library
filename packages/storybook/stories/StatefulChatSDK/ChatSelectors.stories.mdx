...
The `messageThreadSelector` provides data props needed to render a [MessageThread](?path=/docs/ui-components-messagethread--message-thread-component) component.

```tsx
import { ChatClientState, messageThreadSelector } from '@azure/acs-chat-selector';

const state: ChatClientState = 'CHAT_APP_DATA_STORE';
const threadId = 'CHAT_THREAD_ID';
const messageThreadProps = messageThreadSelector(state, { threadId });
