...
The `Declarative Chat SDK` is a stateful Chat SDK that allows you to use `@azure/communication-chat` SDK in a stateful manner.
...
It is also recommended to install the `@azure/communication-common` library that provides commonly used functionality.
...
Calling a client method like `chatThreadClient.updateThread({ topic: 'newTopicName' })` will not return anything. Instead, to view the changes triggered by such a client method, you will have to subscribe to the `onStateChange` events as sown below.
