...
To power Composites, the UI library provides `Adapters`, one for the `CallComposite` [here](./?path=/docs/composites-call-basicexample--basic-example) and one for `ChatComposite`
...
To initialize the call adapter, you will be required to provide a call locator with the information of the group call or meeting you are trying to join.
...
To initialize the chat adapter, you will be required to provide an endpoint from your Azure Communication Services resource on [Azure Portal](https://portal.azure.com).
