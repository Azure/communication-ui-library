...
To use the LocalizationProvider, import it from `@azure/communication-react` and wrap it around ACS UI SDK components. Then assign a `Locale` to the `locale` prop.
