...
If you want to localize your composite, assign a `ComponentLocale` to the `locale` prop of the composite. You may import one of our composite locales like
`COMPOSITE_LOCALE_FR_FR` for French (France) in the example below.

<Source code={LocalizedComposites} />

## Available locales

The following is a table of our available locales for components and composites which can be imported from our package. The component strings are passed as
...
Note: More locales will be supported in future releases.
