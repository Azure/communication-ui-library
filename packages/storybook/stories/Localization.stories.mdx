...
`LocalizationProvider` is a wrapping component that sets all the strings for ACS components using a `Locale`. We have a set of
locales available for import to use. LocalizationProvider is not required as all components default to using our English (US) locale.

### Usage

To use the LocalizationProvider, simply import it from `@azure/communication-react` and wrap it around ACS UI SDK components,
For the example below we will localize buttons in a `ControlBar` in French (France) and import that locale `fr_FR` from `@azure/communication-react`.
...
with strings from our British English locale but you want to change the label of `CameraButton` to "Start" when its state is off.

### Option 1: Import and customize locales

The first option is to import and customize locales. First, import `en_GB` locale from `@azure/communication-react` and then reassign the
...
`Locale` stores all strings in the `strings` property. The component name is the first identifer and the string key is the second identifier such
that each string can be accessed in this pattern `strings.<componentName>.<stringKey>`. If you are using typescript and have intellisense available
in your IDE, it will be easy to discover all the strings.

### Option 2: Component strings prop

The second option is also easy to pass custom strings directly through the `strings` prop. Each component in ACS has an optional `strings`
property that interfaces all strings that can be overriden in that component. Like Option 1, import `en_GB` from `@azure/communication-react`.
Then assign the string directly through `CameraButton` like shown below. This option would not be recommended if a component is used multiple
times as you would need to assign the `strings` multiple times.
...
## Managing expansive translations

There are cases when translations can be expansive and make components overall look awkward like the `ControlBar` below using strings from a German locale.
...
2. Using styling to set limits and truncating expansive translations. The following styling in the example below is passed to each button.
...
All ACS UI SDK components will be "flipped" just like the example `ControlBar` in Arabic below.
