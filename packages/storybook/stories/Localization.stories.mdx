...
import LocalizedComposites from '!!raw-loader!./QuickStarts/snippets/LocalizedComposites.snippet.tsx';
...
The second option is to pass custom strings directly through the `strings` prop. Strings that can be overriden for a UI Library
component have an optional `strings` property that interfaces all. There is no need to use `LocalizationProvider` because all
