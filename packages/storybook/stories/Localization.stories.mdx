...
For the example below, we will localize buttons in a `ControlBar` in French (France) by also importing locale `COMPONENTS_LOCALE_FR_FR` from `@azure/communication-react`.
...
If you want to localize your composite, assign a `ComponentLocale` to the `locale` prop of the composite. You may import one of our locales like `COMPONENTS_LOCALE_FR_FR` for French (France)
...
The first option is to import and customize locales. First, import `COMPONENTS_LOCALE_EN_US` locale from `@azure/communication-react` and then reassign the
