...
UI Library will provide out of the box support for some languages and capabilities such as right-to-left (RTL).
...
The following is a table of locales for components and composites which can be imported from our package. The component strings are passed as
a prop to `LocalizationProvider` while the composite strings are passed as a prop to `ChatComposite` or `CallComposite`.
...
Note: All Azure Communication Services UI Library components will have aria labels with the full text as part of our commitment to accessibility.

3. Using styling to reduce font size.

## Setting rtl

To set RTL of Azure Communication Services UI Library components for rtl languages, use the `FluentThemeProvider` and set the `rtl` prop to true like in
the example below.
