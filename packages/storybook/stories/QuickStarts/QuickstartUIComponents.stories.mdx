...
UI Library components come in two flavors: UI components and Composite components.
...
For Calling scenarios, we'll use the [`GridLayout`](./?path=/story/ui-components-gridlayout--grid-layout-component) and [`ControlBar`](./?path=/story/ui-components-controlbar--control-bar) components.
...
For Chat scenarios, we will use the [`MessageThread`](./?path=/docs/ui-components-chatthread--chat-thread-component) and [`SendBox`](./?path=/docs/ui-components-sendbox--send-box-story-book-component) components.
...
If you want to clean up and remove a Communication Services resource, you can delete the resource or resource group.
