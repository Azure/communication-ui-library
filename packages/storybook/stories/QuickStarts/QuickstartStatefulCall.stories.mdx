...
import CallComponentsStateful from '!!raw-loader!./snippets/CallComponentsStateful.snippet.tsx';
import CallAppStatefulComplete from '!!raw-loader!./snippets/CallAppStatefulComplete.snippet.tsx';

<Meta id="quickstarts-statefulcallclient" title="Stateful Client/Get Started (Call) " />
...
Second, the `CallClientProvider` which provides state from the `statefulCallClient` to the components.
For this provider, we will pass the `statefulCallClient` as a prop to the provider.
Third, the `CallAgentProvider` which provides state from the `CallAgent` to the components.
For this provider, we will pass the `CallAgent` as a prop to the provider.
Finally, we will add the 'CallProvider' which provides the `Call` objects state down to the components.
...
For Chat, we will use the [VideoGallery](./?path=/story/ui-components-videogallery--video-gallery) and [ControlBar](./?path=/story/ui-components-controlbar--control-bar) components.
