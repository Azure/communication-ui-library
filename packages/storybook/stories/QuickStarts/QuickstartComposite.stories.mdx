...
import LocalizedComposites from '!!raw-loader!./snippets/LocalizedComposites.snippet.tsx';

<Meta id="quickstarts-composites" title="Quickstarts/Get started with Composites" />
...
At the end of this process, you should have a full application inside of the folder `ui-toolkit-quickstart-composites`.
...
| Name                                                          | Description                                                                                  |
| ------------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| [CallComposite](/?path=/story/composites-call--basic-example) | Composite component that renders a calling experience with participant gallery and controls. |
| [ChatComposite](/?path=/story/composites-chat--basic-example) | Composite component that renders a chat experience with chat thread and input                |
...
Go to the `src` folder inside of `ui-toolkit-quickstart-composites` and look for the file `app.tsx`.
...
If you want to localize your composite, assign a `Locale` to the `locale` prop. You may import one of our locales like `fr_FR` for French (France) in the example below.

<Source code={LocalizedComposites} />

To learn more about locales, go to the [Localization](/?path=/story/localization--page) resource. Information covered there includes the our available locales, customization, and setting RTL.
