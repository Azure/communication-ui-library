...
First install `uuid` npm package to generate a group ID for the upcoming adapter code by running the following command:
...
Then go to the `src` folder inside of `ui-library-quickstart-composites` and look for the file `App.tsx`.
