...
UI Toolkit supports a number of use cases across calling and chat experiences.
These capabilities extend across both UI Components and Composites.
For Composites, these capabilities are built directly in and exposed when the composite is integrated into an application.
For UI Components, these capabilities are exposed through a combination of UI functionality and underlying state management.
To take full advantage of these capabilities we recommend to use the UI Components in conjunction with the stateful call and chat clients.
...
An Azure Communication Services identity is required to initialize the UI Framework and authenticate to the service. For more information on authentication, see [Authentication](https://docs.microsoft.com/azure/communication-services/concepts/authentication) and [Access Tokens](https://docs.microsoft.com/azure/communication-services/quickstarts/access-tokens?pivots=programming-language-javascript)

## Customization

UI Toolkit exposes key customization use cases to developer so that they can modify components to fit their look and feel of their application.
...
As part of the decoupled state management architecture of the UI Toolkit, developers are able to access the stateful calling and chat clients directly.
