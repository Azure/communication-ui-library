...
| [Teams Interop](https://docs.microsoft.com/en-us/azure/communication-services/concepts/teams-interop)     | Call Lobby                                             |
...
For Interop scenarios, developers can leverage UI Library Components to join Teams meetings through Azure Communication Services.
To enable this, developers can either levarage calling and chat composites directly or use UI Components to build a custom experience. 
...
| ------------------------------------------------ | ---------- | ------------- | ---------------------------------------------------------------------------- |
| Call/Chat client state can be accessed           | X          | X             | Check whether a all is connected or see the participants in a thread         |
| Commands can be sent to the call and chat client | X          | X             | Mute or Unmute the call client or send messages on behalf of the user        |
| Client events can be accessed and handled        | X          | X             | Handle a participant join a call or intercept a message coming into a thread |
| UI events can be accessed and handled            | X          | X             | Handle mute button click or chat send button to add your own logic           |
