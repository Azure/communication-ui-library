...
We can see this error at times when trying to construct our `AzureCommunicationCallAdapter` or `StatefulCallClient` using some of our numerous factory functions. This can be caused by passing the arguments to these functions
more than once or too often. We recommend passing args to these once, or disposing of the adapter or stateful client before attempting to create a new one.
