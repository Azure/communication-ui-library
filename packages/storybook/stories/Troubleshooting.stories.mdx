...
This is an issue when using a mobile browser and the user closes the browser. When they do this
the browser will load the page cached which all of the subscriptions will be lost. To fix this as the
...
This `useEffect` will add and event listener for the `pageshow` event, this event when triggered
