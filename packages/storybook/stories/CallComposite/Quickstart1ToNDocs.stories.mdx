...
import { StartPSTNCall } from './StartPSTNCall.stories';
...
Azure Communication Services call service. Currently, Inbound calls (receiving calls from outbound ACS calls) is not supported by
the UI Library. However, developers are able to create inbound scenarios using our [Call stateful Client](./?path=/docs/quickstarts-statefulcallclient--page).
...
_Unfortunately, users are not able to join into an existing call through a copy invite link for 1:N and PSTN scenarios at this time._
