...
Building visual communication experiences can be challenging.
There are many moving pieces including dynamic remote video galleries, local user controls, rendering incoming chat messages, etc.
On top of that developers must worry about accessibility and localization for this experiences.
Azure Communication Services UI Library makes it easy for you to build visual communications user experiences.
...
These components are turn-key solutions that implement end-to-end communication scenarios like [calling](./?path=/story/composites-call-basicexample--basic-example) and [chat](./?path=/story/composites-chat-basicexample--basic-example).
It takes only a couple lines of code to add and configure.
Composites feature support basic customization options like theming, styling icons and fonts among other capabilities.
...
They are deeply customizable as they not only offer theming and styling, but additional overrides to define methods for rendering the experience for example defining the rendering of a chat message based on who sent it.
...
| UI SDK - Chat    | Firefox\*, Chrome\*, new Edge | Firefox\*, Chrome\*, Safari\* | Chrome\* | Chrome\* | Safari\*   | Safari\*\*\* |
