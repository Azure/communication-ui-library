...
During an active call, participants can transfer another participant to another user or number. The transfer process
starts when one participant requests a transfer to the another participant which they can accept or reject. In the
current beta version, ACS users cannot request to transfer other participants yet but they can receive transfer
requests. Right now only Teams users can request to transfer other participants.
...
when using the [useAzureCommunicationCallAdapter](./?path=/docs/composite-adapters--page#useazurecommunicationcalladapter)
...
The id of a Teams user can be obtained by using the
...
The id should be in the API response like highlighted in the example shown below:
...
shows an example of a `TransferRequestedListener` that accepts all events.

<Source code={TransferSnippetText} />

The `afterCallAdapterCreate` callback in the above snippet can be modified to handle the transfer request such that audio
and video states from the original call are maintained after transfer.
...
Here is another example of handling transfer requests by using a dialog to prompt the user to accept or reject the
transfer request.
