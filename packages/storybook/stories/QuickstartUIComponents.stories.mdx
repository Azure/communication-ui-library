...
Packages are not published to the npm registry during the private preview.
Instead, download the `private-preview.zip` archive from the [latest private preview release on GitHub][private-preview-release]
into the `ui-toolkit-quickstart` directory. Unzip the downloaded archive and install all the npm packages inside.

e.g., if you unzipped into the `ui-toolkit-quickstart/private-preview`:

```bash

# Inside the ui-toolkit-quickstart directory...

npm install --save ./private-preview/azure-acs-calling-declarative-1.0.0-beta.tgz ./private-preview/azure-acs-chat-declarative-1.0.0-beta.tgz ...

```

The exact version numbers of the included packages may be different, please list all `.tgz` files in the archive.

The `--save` option lists the library as a dependency in your **package.json** file.

[private-preview-release]: https://github.com/Azure/communication-ui-sdk/releases/latest
...
| [MessageThread](/?path=/docs/ui-components-chatthread--chat-thread-component) | Base component that renders a chat thread with typing indicators, read receipts, etc.    |
