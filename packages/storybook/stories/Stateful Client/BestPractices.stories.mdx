...
1. _Rendering a RemoteVideoStreamState_
   ...
   If you cannot await the result of `createView`, it is fine to call `createView` again, `createView` will no-op the second call, then at some point you can check the state for the view.
...
3. _Rendering a LocalVideoStreamState not in a call_
...
To make Stateful Calling easier to use we added a `screenshareRemoteParticipant` property to `CallState`.
...
## Detecting transcription and recording in Teams Interop scenarios
