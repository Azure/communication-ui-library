...
Developer has access to `statefulCallClient` and `CallAgent`. The stateful clients are a 1:1 representation.
...
Developer has access `statefulChatClient` and `chatThreadClient`. The stateful clients are a 1:1 representation.
For state to be tracked, developers should use the stateful clients to query the service and listen to events.
