...
If you are using React component from our communication-react components, `usePropsFor` is the recommended React hook to use.
It provides all the required props to power a UI component from the stateful client.
...
`usePropsFor` is context-aware. It picks out props from the relevant provider for the component. One exception is `ParticipantList` -- a component applicable to both chat and calling settings. Thus, `ChatClientProvider` and `CallClientProvider` both provide props for this component\*. Thus, `usePropsFor` can not determine which provider should be used to satisfy the props. You must explicitly set the target to use:
