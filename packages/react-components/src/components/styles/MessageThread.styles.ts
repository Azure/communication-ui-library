...
const bodyWithPlaceholderImage = (myMessage: boolean = false): GriffelStyle => {
  return {
    '& img[src=""]': {
      display: 'block',
      position: 'relative',
      marginBottom: '5px',
      width: myMessage ? '12rem' : undefined,
      height: myMessage ? '12rem' : undefined
    },
    '& img[src=""]:after': {
...
export const useChatMessageStyles = makeStyles({
  ...
  bodyWithPlaceholderImage: bodyWithPlaceholderImage(),
  bodyWithoutAvatar: {
    marginTop: '0.125rem'
  },
  bodyWithAvatar: {
