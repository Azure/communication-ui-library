...
export const FloatingLocalVideoLayout = (props: FloatingLocalVideoLayoutProps): JSX.Element => {
  ...
  const noVideosOn = remoteParticipants.filter((p) => p.videoStream?.isAvailable).length === 0;
  const { gridParticipants, overflowGalleryParticipants } = useOrganizedParticipants({
    remoteParticipants,
    dominantSpeakers,
    maxRemoteVideoStreams: noVideosOn ? MAX_GRID_PARTICIPANTS_NOT_LARGE_GALLERY : maxRemoteVideoStreams,
