...
class MessageRouter {
  ...
  addAnalyzerIssue(messageId, messageText, astDeclarationOrSymbol, properties) {
    ...
    const extractorMessage = this.addAnalyzerIssueForPosition(
      messageId,
      messageText,
  ...
  logError(messageId, message, properties) {
    this._handleMessage(
      new ExtractorMessage_1.ExtractorMessage({
    ...
    switch (message.category) {
      ...
      case 'console' /* Console */:
        throw new node_core_library_1.InternalError(
          'ExtractorMessageCategory.Console is not supported with IReportingRule'
        );
    }
  }
  /**
