...
class AstSymbolTable {
  constructor(program, typeChecker, packageJsonLookup, bundledPackageNames, messageRouter) {
    ...
    this._packageMetadataManager = new PackageMetadataManager_1.PackageMetadataManager(
      packageJsonLookup,
      messageRouter
    );
  ...
  tryGetEntityForNode(identifier) {
    if (!this._entitiesByNode.has(identifier)) {
      throw new node_core_library_1.InternalError(
        'tryGetEntityForIdentifier() called for an identifier that was not analyzed'
      );
    }
      ...
      case ts.SyntaxKind.TypeQuery: // represents for "typeof X" as a type
        {
          ...
          const identifierNode = TypeScriptHelpers_1.TypeScriptHelpers.findFirstChildNode(
            node,
  ...
  _fetchAstSymbol(options) {
    ...
    if (TypeScriptHelpers_1.TypeScriptHelpers.isFollowableAlias(followedSymbol, this._typeChecker)) {
      ...
      throw new node_core_library_1.InternalError(
        'AstSymbolTable._fetchAstSymbol() cannot be called with a symbol alias'
      );
    }
      ...
      if (!nominalAnalysis) {
        ...
        if (arbitraryDeclaration) {
          ...
          if (arbitraryParentDeclaration) {
            const parentSymbol = TypeScriptHelpers_1.TypeScriptHelpers.getSymbolForDeclaration(
            ...
            if (!parentAstSymbol) {
              throw new node_core_library_1.InternalError(
                'Unable to construct a parent AstSymbol for ' + followedSymbol.name
              );
            }
          }
        }
