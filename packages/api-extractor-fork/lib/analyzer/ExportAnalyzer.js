...
class ExportAnalyzer {
  ...
  _getModuleSymbolFromSourceFile(sourceFile, moduleReference) {
    const moduleSymbol = TypeScriptInternals_1.TypeScriptInternals.tryGetSymbolForDeclaration(
      sourceFile,
      this._typeChecker
    );
    ...
    if (moduleReference !== undefined) {
      ...
      if ((moduleReference.moduleSpecifierSymbol.flags & ts.SymbolFlags.Alias) !== 0) {
        ...
        let followedSymbol = TypeScriptInternals_1.TypeScriptInternals.getImmediateAliasedSymbol(
  ...
  fetchReferencedAstEntity(symbol, referringModuleIsExternal) {
    ...
    if (referringModuleIsExternal) {
      ...
      for (;;) {
        ...
        const currentAlias = TypeScriptInternals_1.TypeScriptInternals.getImmediateAliasedSymbol(
          current,
          this._typeChecker
        );
  ...
  _tryMatchExportDeclaration(declaration, declarationSymbol) {
    const exportDeclaration = TypeScriptHelpers_1.TypeScriptHelpers.findFirstParent(
      declaration,
  ...
  _tryMatchImportDeclaration(declaration, declarationSymbol) {
    const importDeclaration = TypeScriptHelpers_1.TypeScriptHelpers.findFirstParent(
      declaration,
    ...
    if (importDeclaration) {
      ...
      if (declaration.kind === ts.SyntaxKind.ImportSpecifier) {
        ...
        const exportName = importClause.name ? importClause.name.getText().trim() : ts.InternalSymbolName.Default;
        if (externalModulePath !== undefined) {
          return this._fetchAstImport(declarationSymbol, {
    ...
    if (ts.isImportEqualsDeclaration(declaration)) {
      ...
      if (ts.isExternalModuleReference(declaration.moduleReference)) {
        if (ts.isStringLiteralLike(declaration.moduleReference.expression)) {
          ...
          const externalModuleName = TypeScriptInternals_1.TypeScriptInternals.getTextOfIdentifierOrLiteral(
  ...
  _fetchSpecifierAstModule(importOrExportDeclaration, exportSymbol) {
    ...
    const resolvedModule = TypeScriptInternals_1.TypeScriptInternals.getResolvedModule(
