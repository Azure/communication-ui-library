...
class TypeScriptHelpers {
  ...
  static isAmbient(symbol, typeChecker) {
    ...
    if (followedSymbol.declarations && followedSymbol.declarations.length > 0) {
      ...
      const highestModuleDeclaration = TypeScriptHelpers.findHighestParent(
        firstDeclaration,
  ...
  static getModuleSpecifier(nodeWithModuleSpecifier) {
    if (nodeWithModuleSpecifier.kind === ts.SyntaxKind.ImportType) {
      ...
      return nodeWithModuleSpecifier.argument.literal.text.trim();
    }
    // Node is a declaration
    if (nodeWithModuleSpecifier.moduleSpecifier && ts.isStringLiteralLike(nodeWithModuleSpecifier.moduleSpecifier)) {
      return TypeScriptInternals_1.TypeScriptInternals.getTextOfIdentifierOrLiteral(
