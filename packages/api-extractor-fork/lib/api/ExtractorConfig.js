...
class ExtractorConfig {
  ...
  static tryLoadForFolder(options) {
    ...
    const packageFolder = packageJsonFullPath ? path.dirname(packageJsonFullPath) : undefined;
  ...
  static prepare(options) {
    ...
    try {
      ...
      const mainEntryPointFilePath = ExtractorConfig._resolvePathWithTokens(
        'mainEntryPointFilePath',
      ...
      const tsconfigFilePath = ExtractorConfig._resolvePathWithTokens(
        'tsconfigFilePath',
      ...
      if (configObject.apiReport) {
        ...
        const reportFilename = ExtractorConfig._expandStringWithTokens(
          'reportFileName',
        ...
        const reportFolder = ExtractorConfig._resolvePathWithTokens(
          'reportFolder',
          configObject.apiReport.reportFolder,
          tokenContext
        );
        const reportTempFolder = ExtractorConfig._resolvePathWithTokens(
          'reportTempFolder',
      ...
      if (configObject.tsdocMetadata) {
        ...
        if (tsdocMetadataEnabled) {
          ...
          if (tsdocMetadataFilePath.trim() === '<lookup>') {
            ...
            tsdocMetadataFilePath = PackageMetadataManager_1.PackageMetadataManager.resolveTsdocMetadataPath(
    ...
    if (!tsdocConfigFile) {
      ...
      let packageTSDocConfigPath = tsdoc_config_1.TSDocConfigFile.findConfigPathForFolder(
      ...
      if (!packageTSDocConfigPath || !node_core_library_1.FileSystem.exists(packageTSDocConfigPath)) {
        ...
        if (!node_core_library_1.FileSystem.exists(packageTSDocConfigPath)) {
          throw new node_core_library_1.InternalError(
            'Unable to load the built-in TSDoc config file: ' + packageTSDocConfigPath
          );
        }
      }
    ...
    return new ExtractorConfig(
      Object.assign(Object.assign({}, extractorConfigParameters), { tsdocConfigFile, tsdocConfiguration })
    );
  }
 ...
 */
ExtractorConfig.jsonSchema = node_core_library_1.JsonSchema.fromFile(
...
ExtractorConfig._defaultConfig = node_core_library_1.JsonFile.load(
