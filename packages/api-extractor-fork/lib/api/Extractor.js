...
class Extractor {
  ...
  static invoke(extractorConfig, options) {
    ...
    if (extractorConfig.tsdocConfigFile.filePath && !extractorConfig.tsdocConfigFile.fileNotFound) {
      if (
        !node_core_library_1.Path.isEqual(
          extractorConfig.tsdocConfigFile.filePath,
          ExtractorConfig_1.ExtractorConfig._tsdocBaseFilePath
        )
      ) {
        messageRouter.logVerbose(
          'console-using-custom-tsdoc-config' /* UsingCustomTSDocConfig */,
    ...
    if (messageRouter.showDiagnostics) {
      ...
      const serializedCompilerOptions = MessageRouter_1.MessageRouter.buildJsonDumpObject(
        compilerState.program.getCompilerOptions()
      );
      ...
      const serializedTSDocConfig = MessageRouter_1.MessageRouter.buildJsonDumpObject(
    ...
    if (extractorConfig.docModelEnabled) {
      messageRouter.logVerbose(
        'console-writing-doc-model-file' /* WritingDocModelFile */,
    ...
    if (extractorConfig.apiReportEnabled) {
      ...
      if (node_core_library_1.FileSystem.exists(expectedApiReportPath)) {
        ...
        if (
          !ApiReportGenerator_1.ApiReportGenerator.areEquivalentApiFileContents(
          ...
          if (!localBuild) {
            ...
            messageRouter.logWarning(
              'console-api-report-not-copied' /* ApiReportNotCopied */,
            ...
            messageRouter.logWarning(
              'console-api-report-copied' /* ApiReportCopied */,
          ...
          messageRouter.logVerbose(
            'console-api-report-unchanged' /* ApiReportUnchanged */,
        ...
        if (!localBuild) {
          ...
          if (!node_core_library_1.FileSystem.exists(expectedApiReportFolder)) {
            messageRouter.logError(
              'console-api-report-folder-missing' /* ApiReportFolderMissing */,
            ...
            messageRouter.logWarning(
              'console-api-report-created' /* ApiReportCreated */,
    ...
    if (extractorConfig.rollupEnabled) {
      Extractor._generateRollupDtsFile(
        ...
        DtsRollupGenerator_1.DtsRollupKind.PublicRelease,
        extractorConfig.newlineKind
      );
      Extractor._generateRollupDtsFile(
        collector,
        extractorConfig.betaTrimmedFilePath,
        DtsRollupGenerator_1.DtsRollupKind.BetaRelease,
        extractorConfig.newlineKind
      );
      Extractor._generateRollupDtsFile(
        collector,
        extractorConfig.untrimmedFilePath,
        DtsRollupGenerator_1.DtsRollupKind.InternalRelease,
        extractorConfig.newlineKind
      );
    }
    if (extractorConfig.tsdocMetadataEnabled) {
      // Write the tsdoc-metadata.json file for this project
      PackageMetadataManager_1.PackageMetadataManager.writeTsdocMetadataFile(
    ...
    try {
      ...
      if (packageJson && packageJson.version && semver.valid(packageJson.version)) {
        ...
        if (theirMajor > ourMajor || (theirMajor === ourMajor && theirMinor > ourMinor)) {
          messageRouter.logInfo(
            'console-compiler-version-notice' /* CompilerVersionNotice */,
  ...
  static _generateRollupDtsFile(collector, outputPath, dtsKind, newlineKind) {
    if (outputPath !== '') {
      collector.messageRouter.logVerbose(
        'console-writing-dts-rollup' /* WritingDtsRollup */,
