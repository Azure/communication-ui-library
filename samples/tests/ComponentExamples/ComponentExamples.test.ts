...
test.describe('Component Examples Test', () => {
  ...
  test('Whether html page is loaded right with 2 composites', async ({ page }) => {
    expect(await page.waitForSelector('text=Connect components to StatefulClient')).toBeTruthy();
    expect(await page.waitForSelector('text=Render single video stream in your own component')).toBeTruthy();
    ...
    await page.waitForFunction(() => {
      ...
      return document.querySelectorAll('video').length === 1 && videoNode?.readyState === 4;
    });

    expect(await page.screenshot({ fullPage: true })).toMatchSnapshot('componentExamples-localVideoStarted.png');

    // Stop local video using CameraButton from UI Library
