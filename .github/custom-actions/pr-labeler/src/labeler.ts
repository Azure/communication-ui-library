...
export async function run() {
  try {
    ...
    const changedFiles: string[] = await getChangedFiles(client, prNumber, onlyLastCommit);
    const labelGlobs: Map<string, StringOrMatchConfig[]> = await getLabelGlobs(client, configPath);
...
async function getChangedFiles(client: github.GitHub, prNumber: number, onlyLastCommit: boolean): Promise<string[]> {
...
function getLabelGlobMapFromObject(configObject: any): Map<string, StringOrMatchConfig[]> {
...
export function checkGlobs(changedFiles: string[], globs: StringOrMatchConfig[]): boolean {
...
async function addLabels(client: github.GitHub, prNumber: number, labels: string[]) {
...
async function removeLabels(client: github.GitHub, prNumber: number, labels: string[]) {
  await Promise.all(
    labels.map((label) =>
